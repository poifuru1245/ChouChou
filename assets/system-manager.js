import "./admin.js";
import { createSystemItem, deleteSystemItem, reorderSystemItems, subscribeSystemItems, updateSystemItem } from "./services/systemService.js";
...
form?.addEventListener("submit", async (event) => {
  ...
  const payload = { type: data.get("type"), label: String(data.get("label") || "").trim(), price: String(data.get("price") || "").trim(), isPublished: data.get("isPublished") === "on" };
  if (!payload.label || !payload.price) return setMessage("項目名と金額を入力してください。", "error");
  if (editingId) await updateSystemItem(editingId,payload);
  else await createSystemItem({...payload,displayOrder:items.length + 1});
...
list?.addEventListener("click", async (event) => {
  ...
  if (button.dataset.action === "toggle") await updateSystemItem(item.id,{isPublished:item.isPublished===false});
  if (button.dataset.action === "delete" && confirm(`${item.label}を削除しますか？`)) await deleteSystemItem(item.id);
});
list?.addEventListener("dragstart", (event) => { const card=event.target.closest("[data-id]"); draggedId=card?.dataset.id||""; });
list?.addEventListener("dragover", (event) => event.preventDefault());
list?.addEventListener("drop", async (event) => { event.preventDefault(); const target=event.target.closest("[data-id]"); if(!target||!draggedId||target.dataset.id===draggedId)return; const ids=items.map(i=>i.id); const from=ids.indexOf(draggedId); const to=ids.indexOf(target.dataset.id); ids.splice(to,0,ids.splice(from,1)[0]); await reorderSystemItems(ids); draggedId=""; setMessage("表示順を保存しました。","success"); });
