...
function render(){
  ...
  const ranked = state.casts.filter((cast)=>cast.isPublished!==false && getImage(cast)).map((cast)=>({cast,count:counts.get(cast.id) ?? Number(cast.weeklyViews ?? cast.viewCount ?? 0)})).sort((a,b)=>b.count-a.count || Number(a.cast.displayOrder??9999)-Number(b.cast.displayOrder??9999)).slice(0,5);
  if(!ranked.length){list.innerHTML='<p class="no-cast">ランキングを準備中です。</p>';return;}
  list.innerHTML=ranked.map(({cast,count},index)=>`<article class="v6-ranking-card card-premium"><a href="cast-detail.html?id=${encodeURIComponent(cast.id)}&name=${encodeURIComponent(cast.name||"")}" aria-label="${escapeAttr(cast.name||"キャスト")}のプロフィール"><span class="v6-rank-number">${index+1}<small>RANK</small></span><img src="${escapeAttr(getImage(cast))}" alt="${escapeAttr(cast.name||"")}" loading="lazy" decoding="async"><span class="v6-ranking-glass"><strong>${escapeHtml(cast.name||"CAST")}</strong><small>${count.toLocaleString("ja-JP")} views</small></span></a></article>`).join("");
}
