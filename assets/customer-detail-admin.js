...
const state = { customers:[], casts:[], reservations:[], sales:[], ready:new Set() };
...
function initialize() {
  ...
  subscribeCollection("casts", (rows) => { state.casts = rows; markReady("casts"); }, handleError);
...
function markReady(key) { state.ready.add(key); if (state.ready.size === 4) render(); }
...
function render() {
  ...
  const completed = reservations.filter((item) => ["来店", "会計済", "完了"].includes(item.status));
  ...
  document.getElementById("customerProfileList").innerHTML = profile("ランク", customer.rank) + profile("氏名", customer.name) + profile("ふりがな", customer.kana) + profile("ニックネーム", customer.nickname) + profile("電話番号", customer.phone) + profile("LINE ID", customer.lineId) + profile("誕生日", formatDate(customer.birthday, "")) + profile("職業", customer.occupation);
  renderCastRelations(customer);
  renderTimeline(reservations, sales);
...
function renderTimeline(reservations, sales) {
  const events = [
